...
- **Block-editor term trees follow the edited post's language**: Gutenberg loads taxonomy collections over REST, where `is_admin()` is false, so the context used to resolve to the FRONTEND language and a translated post's editor showed the wrong-language tree (none of the post's terms matched a checkbox — the empty-save wipe contained by 1.8.2). `WP_LOC_Terms::get_rest_edited_post_language()` recovers the post from an explicit `?post=` arg or the editor referer (`post.php?post=ID`); it is memoized because `get_term` consults it once per term — but **only once `REST_REQUEST` is actually defined**: term queries also run earlier in the same request (init and friends), and caching the "no editor context" answer from one of those poisoned every later call and brought the whole bug back. **Two places must honor it, and fixing only one leaves the bug visible**: `get_context_language()` (so `filter_terms_clauses` scopes the SQL) AND `adjust_term_to_current_language()`, which must return the term untouched in that context — it is hooked on `get_term`, `WP_Term_Query` maps every result row through `get_term()`, so it silently re-mapped each correctly-scoped term back to the default language. Outside an editor REST context both behave exactly as before; the classic editor metabox renders on a real admin screen and was never affected.
