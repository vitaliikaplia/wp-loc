...
- **Block-editor term trees follow the edited post's language**: Gutenberg loads taxonomy collections over REST, where `is_admin()` is false, so the generic context used to resolve to the FRONTEND language and a translated post's editor showed the wrong-language tree (none of the post's terms matched a checkbox — the empty-save wipe contained by 1.8.2). `WP_LOC_Terms::get_rest_edited_post_language()` recovers the post from an explicit `?post=` arg or the editor referer (`post.php?post=ID`) and scopes `filter_terms_clauses` to that post's language; outside that context the old resolution applies. The classic editor metabox renders on a real admin screen and was never affected.
