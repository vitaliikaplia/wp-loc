...
includes/
  ...
  class-wp-loc-menu-sync.php        → Multilingual > WP Menus Sync page (AJAX preview/apply for menu structure sync)
...
- **AJAX operations**: Language sort order auto-saves on drag. Single translation creation via `+` button in metabox (no page reload). `WP Menus Sync` preview/apply also runs via AJAX.
